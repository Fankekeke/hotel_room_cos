        ...
        <a-col :span="12" v-for="(item, index) in cartData" :key="index" style="margin-bottom: 15px">
          <div style="width: 100%;margin-bottom: 15px;text-align: left;box-shadow: rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px;">
            <a-card :bordered="false" hoverable>
              ...
              <a-card-meta :title="item.name + ' * '+ item.total + '件'" :description="item.remark.slice(0, 18)+'...'" style="margin-top: 10px"></a-card-meta>
              <div style="font-size: 12px;font-family: SimHei;margin-top: 8px">
                <span>{{ item.model }}</span> |
                <span  style="margin-left: 2px">库存：{{ item.stockNum }}</span> |
                <span style="color: #f5222d; font-size: 13px;float: right">{{ item.price }}元</span>
              </div>
            </a-card>
          </div>
        </a-col>
  ...
  methods: {
    handleSubmit () {
      ...
      this.cartData.forEach(e => {
        goodsList.push({commodityId: e.id, outNum: e.total, itemPrice: e.unitPrice})
      })
      let values = {userId: this.currentUser.userId, goodsList: JSON.stringify(goodsList)}
      this.$post('/cos/purchase-record', values).then((r) => {
        this.$emit('success')
      })
    },
    onClose () {
